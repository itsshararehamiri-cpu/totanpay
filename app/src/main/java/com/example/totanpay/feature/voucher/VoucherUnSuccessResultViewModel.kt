...
@HiltViewModel
class VoucherUnSuccessResultViewModel @Inject constructor(private val mainRepository: MainRepository,private val deviceSettingsRepository: DeviceSettingsRepository) :
    ViewModel() {
    private val _uiState = MutableStateFlow(VoucherUnSuccessUiState())
    val uiState: StateFlow<VoucherUnSuccessUiState> = _uiState
    init {
    ...
    fun init(response: String) {
        viewModelScope.launch {
            ...
            _uiState.update { it.copy(autoPrint = true) }
...
data class VoucherUnSuccessUiState(
