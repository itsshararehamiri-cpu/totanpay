...
@HiltViewModel
class BillPaymentSuccessResultViewModel @Inject constructor(private val mainRepository: MainRepository
,private val printCustomerSettingsRepository: PrintCustomerSettingsRepository,
    private val deviceSettingsRepository: DeviceSettingsRepository) :
    ViewModel() {
    private val _uiState = MutableStateFlow(BillPaymentSuccessResultUiState())
    val uiState: StateFlow<BillPaymentSuccessResultUiState> = _uiState
    ...
    fun init(response: String) {
        viewModelScope.launch {
            val result=Gson().fromJson(response,
                ResponseTransaction::class.java)
            var printStatus =
            ...
            _uiState.update { it.copy(result = result.copy(date = getPersianDate(result.date)),printStatus=printStatus) }
...
data class BillPaymentSuccessResultUiState(
    val result: ResponseTransaction?=null,
    val error:String="", val printStatus: PrintStatus = PrintStatus.ALWAYS_PRINTING,
