...
import com.example.totanpay.common.receipt.getFontSizeAmount
...
@Composable
fun BillPaymentUnSuccessResult(
    viewModel: BillPaymentUnSuccessResultViewModel,
    response: String,
    ...
    if (uiState.result != null)
    {
        ReceiptResultContainer(printTitle = stringResource(id = R.string.print), showCustomerPrintButton = false,
...
fun UnSuccessReceiptContent(isPaperReceipt: Boolean, result: ResponseTransaction?) {
    ...
    Column(
        ...
        if (result.maskedPan.isNotEmpty())
