...
fun BillPaymentReceiptContent(
    ...
    Column(
        ...
        AddMerchantNamePhone(modifier =   Modifier.rowReceiptWithPSPLogoModifier(isPaperReceipt),
            merchantName = result!!.merchantName,
            merchantPhone = result.merchantPhone,
            isPaperReceipt = isPaperReceipt,
            textColor = firstColor
        )
        AddTypeDateTime(modifier = modifierRowReceipt,
            type = TransactionType.BILL_PAY.title,
            date = result.date,
            time = result.time,
        ...
        AddMaskedPanCardIssuer(modifier = modifierRowReceipt,
            maskedPan = result.maskedPan,
            cardIssuer = result.issuerName,
            textColor = firstColor, isPaperReceipt = isPaperReceipt,
        )
        AddRRNStan(modifier = modifierRowReceipt,rrn = result.rrn, stan = result.trace, textColor = firstColor, isPaperReceipt = isPaperReceipt)
        if (isPaperReceipt)
        {
            AddPSPLog(
                modifier = Modifier.fillMaxWidth(),
