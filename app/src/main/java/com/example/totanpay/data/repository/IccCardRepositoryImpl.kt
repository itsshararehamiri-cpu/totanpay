...
import com.example.totanpay.data.util.ApduUtil.getExponentCommand
import com.example.totanpay.data.util.ApduUtil.getReadPrivateKeyCommand
import com.example.totanpay.data.util.ApduUtil.getVerifyFirstPinCommand
...
class IccCardRepositoryImpl @Inject constructor(
    ...
    private suspend fun verifyFirstPin(pin: String): String? {
        try {
            val responseBuffer: ByteArray =
                device.sendApdu(ISOUtil.hex2byte(getVerifyFirstPinCommand(pin))) ?: return null
    ...
    override suspend fun readPublicKey(): String? {
        return withContext(ioDispatcher){
            try {
                ...
                responseBuffer = device.sendApdu(ISOUtil.hex2byte(getExponentCommand()))
                responseLength = responseBuffer!!.size
                val publicKeyData = ISOUtil.hexString(responseBuffer, 0, responseLength - 2)
                val spec =
                    RSAPublicKeySpec(BigInteger(publicKeyData, 16), BigInteger(publicKeyExponent, 16))
    ...
    override suspend fun readPrivateKey(): PrivateKey? {
      return withContext(ioDispatcher){
           try {
               var responseBuffer: ByteArray? =
                   device.sendApdu(ISOUtil.hex2byte(getReadPrivateKeyCommand()))
               var responseLength = responseBuffer!!.size
