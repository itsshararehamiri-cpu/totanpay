...
class TopupTransaction(
    request: TopupTransactionRequest, private val iMacGenerator: IMacGenerator,
    ...
    override suspend fun buildMessage() {
        with(sendMessage) {
            ...
            pan = (request as TopupTransactionRequest).pan
    ...
    override suspend fun onFail(receivedIsoMessage: IsoMessage?): FaildTransactionResponse {
        if (receivedIsoMessage == null) {
            ...
            println("sssssssssssssspofr->${Gson().toJson(v)}")
            return v
        } else {
            println("begzae9")
