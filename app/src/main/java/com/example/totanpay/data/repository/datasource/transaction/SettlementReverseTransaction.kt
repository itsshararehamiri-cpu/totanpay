...
class SettlementReverseTransaction(
    ...
    saveReverseData,
    saveReverseData,
    updateTransaction = updateTransaction, sendTransactionInQueue = {true}
) {
    ...
    override suspend fun buildMessage() {
        with(sendMessage) {
            mti =
                if ((request as SettlementReverseTransactionRequest).status == 'R') "0420" else "0220"
    ...
    override suspend fun onFail(receivedIsoMessage: IsoMessage?): FailedTransactionResponse {
        ...
        else return FailedTransactionResponse(
            responseCode = receivedIsoMessage.respCode,
            responseMessage = ResponseMessageContainer
                .valueOfLabel(receivedIsoMessage.respCode.toString()).messageId,
            reasonCode = null,
            date = sendMessage.tranDate,
            time = sendMessage.tranTime,
            stan = sendMessage.stan,
