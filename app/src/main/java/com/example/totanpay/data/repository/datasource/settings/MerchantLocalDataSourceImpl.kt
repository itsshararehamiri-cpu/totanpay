...
class MerchantLocalDataSourceImpl @Inject constructor(
    ...
    override fun storeMerchant(merchantId: String?, merchantPhone: String?, merchantName: String?,
        ...
        totanPayPreference.storeEnglishMerchantName(englishMerchantName)
    ...
    override fun getMerchant(): Merchant {
        return Merchant(
            merchantId = totanPayPreference.getMerchantId(),
            merchantPhone = totanPayPreference.getMerchantPhone(),
            merchantName = totanPayPreference.getMerchantName(),
            englishMerchantName=totanPayPreference.getEnglishMerchantName()
        )
    }
    ...
    override fun getApportionments(): List<Apportionment>? {
        return apportionmentDao.getAll()?.map {
            Apportionment(IBAN = it.IBAN, amount = it.amount, bankName = it.bankName, englishBankName = it.englishBankName)
        }
    }
    ...
    override fun storeApportionments(apportionments: List<Apportionment>) {
        ...
        apportionmentDao.insertAll(apportionments.map {
            ApportionmentEntity(IBAN = it.IBAN, amount = it.amount, bankName = it.bankName, englishBankName = it.englishBankName)
        })
    }
    ...
    override fun storePosCode(posCode: String) {
        totanPayPreference.storePosCode(posCode)
    }

    override fun getPosCode(): String {
        return totanPayPreference.getPosCode()
    }
}