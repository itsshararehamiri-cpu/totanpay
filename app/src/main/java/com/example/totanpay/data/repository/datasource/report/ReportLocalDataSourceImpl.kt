...
class ReportLocalDataSourceImpl @Inject constructor(
    ...
    override suspend fun geTransactionBasedStan(
        stan: String,
        traceIsSelected: Boolean
    ): TransactionLog? {
        val transaction = if (traceIsSelected) transactionLogDao.getByStan(stan = stan)
        else transactionLogDao.getByRrn(rrn = stan)
        if (transaction != null) {
